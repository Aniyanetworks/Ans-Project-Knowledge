...
export default function RestrictedTopicsTab() {
  const { user } = useAuth()
            ...
            editingId === topic.id ? (
              ...
              <li key={topic.id} className="flex items-start justify-between gap-3 p-4">
                <div className={`min-w-0 flex-1 ${!topic.is_active ? 'opacity-50' : ''}`}>
                  ...
                  {topic.description && <p className="mt-0.5 text-xs text-slate-500">{topic.description}</p>}
                </div>
                <div className="flex shrink-0 items-center gap-3">
                  ...
                  <button onClick={() => setDeleteTarget(topic)} className="text-xs text-red-400 hover:text-red-600">
                    Delete
                  </button>
                </div>
              </li>
